...
const reviewRecord = await read("docs/validation/records/v2-neon-market-kit.md");
const decisionMatch = reviewRecord.match(/^Decision: (🟢 Pass|🟡 Conditional pass|🔴 Fail)$/m);
const scoreMatch = reviewRecord.match(/^Weighted score: `([0-9]+(?:\.[0-9]+)?)\/100`$/m);
const scoringPerformed = Boolean(decisionMatch && scoreMatch);
if (scoringPerformed && blockers.length) throw new Error("Recorded V2 gate decision conflicts with current preflight blockers.");
const gateDecision = decisionMatch?.[1].replace(/^[^ ]+ /, "").toLowerCase().replaceAll(" ", "-");
...
await write(resolve(output, "V2-E07-test-report.json"), `${JSON.stringify({ command: "npm run validate", status: "pending-final-validation", coverage: ["contracts", "inheritance", "bounds", "states", "sizes", "clipping", "progress-parts", "browser-cli-equivalence", "determinism", "surface-pixel-integrity", "V1-regression"] }, null, 2)}\n`);
await write(resolve(output, "V2-E09-preflight.json"), `${JSON.stringify(scoringPerformed ? { status: "review-complete", blockers, defects: [], scoringPerformed, weightedScore: Number(scoreMatch[1]), gateDecision } : { status: blockers.length ? "blocked-before-scoring" : "ready-for-human-review", blockers, defects: [], scoringPerformed }, null, 2)}\n`);
