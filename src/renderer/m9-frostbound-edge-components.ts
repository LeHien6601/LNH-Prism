...
function responseLayers(instanceId: string, path: string, component: M7AngularComponent): string {
...
function migrate(svg: string, request: M7AngularRequest, framePart = false): string {
  ...
  const responses = responseLayers(`${match[1]}-m9`, match[2], request.component);
