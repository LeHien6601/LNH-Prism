...
| M11-R001 | 🔵 Agent-ready | Replace inherited Frostbound material grammar with deterministic, independently visible stone, dark-wood, moss, and botanical treatments through the shared composition seam. | Source, phone, and thumbnail surfaces show a third family rather than a recolor; all approved bounds, semantic text, stable IDs, geometry, module boundaries, seeds, and reference boundary remain valid. | Focused renderer/material tests, regenerated matrix/portrait/isolates/package receipts, technical preflight, reference-boundary, and control-drift checks. |
