...
| Unity export | 🔵 M4-S1 agent-ready | Option A approved the [M4 implementation specification](implementation/M4_UNITY_EXPORT_IMPLEMENTATION_SPEC.md) and [V4 rubric](validation/V4_UNITY_INTEGRATION_RUBRIC.md) with manifest `1.1`/legacy `1.0` compatibility and authoritative kebab-case naming; contract implementation is next |
...
| Focus | Current value |
|---|---|
| Active milestone | 🔵 **M4 implementation** — definition approved; M4-S1 contract and stable-identity work is agent-ready |
| Next task | 🔵 **Implement M4-S1 contract extensions, semantic validation, naming/GUID rules, canonical fixtures, and migration notes** · 🤖 Agent |
| Next agent-ready task | 🔵 **M4-S1** — legacy manifests remain valid; malformed Unity profiles, settings, collisions, and unsafe paths fail |
...
| P0 | Implement M4-S1 contract extensions, semantic validation, naming/GUID rules, canonical fixtures, and migration notes | 🤖 Agent | 🔵 Agent-ready | Updated validators accept legacy `1.0` and Unity-targeted `1.1`; malformed profiles/settings, unsafe paths, border failures, duplicate/case-folded IDs/paths, and GUID collisions or drift fail |
